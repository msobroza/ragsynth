...
| D27 | *(Phase 5 review)* Fidelity reference: ONE shared anchor subsample (size `min(n_anchor, n_per_arm)`, substream `validator.fidelity`) trimmed per arm; the arm side is subsampled too when it exceeds the reference — equal-n on BOTH sides (SPEC §8) and cross-arm comparability (prototype L860). `wc2st_min_per_side` is validator config, default 30 (SPEC §8's "min 30/side"); small test worlds pass 5 explicitly. | Adversarial audit findings. |
| D28 | *(Phase 5 review)* Per-stratum gate verdicts are reported in `validity.per_stratum[*].passed` (null when n < 30); the headline `gates_passed` stays the aggregate τ/τ_AP check. | SPEC §9 "per stratum" reconciled with small-n strata. |
| D29 | *(Phase 5 review)* Quota allocation is proportional largest-remainder over `tilt_weights` with a deterministic waterfill to the `n_min` floor (not floor+remainder — that systematically shifted above-floor quotas). `drop_index_mask` truncates (`int`), matching the prototype. Curator memorization audits ALL production splits. Judge-family warning compares leading name tokens (gpt-4o ~ gpt-4o-mini). | Audit findings vs SPEC §6.1/§6.6/§6.4. |
| D30 | *(Phase 5 review)* `experiments/*/artifacts/` is gitignored: sklearn KMeans float reductions jitter fitted centers by 1 ULP across runs, churning npz bytes + manifest hashes. metrics.json/report.md/figures stay committed (byte-stable). Same-seed-same-process determinism unaffected. | Audit determinism finding. |
| D31 | *(Phase 5 review)* Known, accepted observations: metric-internal rngs reuse `default_rng(seed)`-style streams (prototype-faithful — changing them breaks byte-equivalence with the vendored ground truth); `config_hash` covers the full config including `artifacts_dir` (determinism is per-config, not per-location); `TrialRecord.timestamp` is wall-clock (unreachable from metrics.json in v1); builtin-`hash` is never used for output ordering. | Audit notes triaged as won't-fix. |
| D32 | *(v2)* **Data layout & fetch.** `scripts/fetch_benchmarks.py <name>` downloads upstream releases into `data/benchmarks/<name>/raw/`; `scripts/convert_benchmark.py <name>` emits `chunks.jsonl`/`queries.jsonl`/`anchor_qrels.jsonl`. `data/benchmarks/` is gitignored; the fetch script records source URL, release sha256, and license text in `data/benchmarks/README.md`. Datasets are data, not runtime deps. | spec01 §3 D32. |
| D33 | *(v2)* **FiQA/NFCorpus chunking.** Native unit = chunk, 1:1 (FiQA passage / NFCorpus abstract); `doc_id` = upstream id. No re-chunking. | spec01 §3 D33. |
| D34 | *(v2)* **LegalBench-RAG chunking.** Deterministic pure-python splitter: 1,000-char windows, 200-char overlap, window end snapped back to the last whitespace; char offsets kept per chunk for D35. | spec01 §3 D34. |
| D35 | *(v2)* **Span→chunk gold rule.** A chunk is gold for a query iff `|span ∩ chunk| ≥ min(0.5 · |span|, 200)` chars for at least one gold span; binary grade 1 (v1 qrel semantics). | spec01 §3 D35. |
| D36 | *(v2)* **Legal sub-corpora merged.** CUAD + ContractNLI + MAUD + PrivacyQA form ONE corpus (`metadata.subcorpus`); 60/25/15 split stratified per sub-corpus (`split_stratify_by`); report carries a per-sub-corpus τ appendix. One dataset ⇒ one report. | spec01 §3 D36. |
| D37 | *(v2)* **Cluster-count ladder.** Default `C=8`; descend over `{8,6,4,2}` while (a) every cluster has ≥ 30 anchor queries and (b) `n_seeds · q_c ≥ 30` per cluster (`n_seeds=240`, λ=0.7) — evaluated a priori, deterministically; chosen `C` logged in `metrics.json["partition_ladder"]` and the report header; a runtime stratum still below 30 falls back to the v1 SPEC §8 wC2ST floor rule (flagged, skipped). | spec01 §3 D37. |
| D38 | *(v2)* **`n_per_arm = 160`** (within the 150–200 §16 band); the curated A2 set doubles as the human precision-audit sample (§11). | spec01 §3 D38. |
| D39 | *(v2)* **Models.** Embedder: `sentence-transformers/all-MiniLM-L6-v2` via the existing `sentence_transformer` registry key (`st` extra). Generator: `Qwen/Qwen2.5-7B-Instruct`; judge: `meta-llama/Llama-3.1-8B-Instruct` — different families, so the §6.4 same-family warning stays quiet by construction, verified by a config test. Both via `openai_compatible`. *(Embedder superseded — see D39-A below.)* | spec01 §3 D39. |
| D39-A | *(2026-07-23, user-directed)* v2 default embedder = `gemini-embedding-2` (google-genai, `output_dimensionality` 768) behind the cached_chroma ChromaDB cache (collection keyed by `sha256(embedder_id)`, metadata records embedder id + dataset). MiniLM/`sentence_transformer` remains available as fallback. Embedding determinism boundary: cached vectors are the artifact; live re-embedding is the documented exception, mirroring D40. | Amendment A1, specs/v2/01-real-benchmarks.md. |
| D40 | *(v2)* **Determinism boundary = transcript replay.** New `cached` ChatModel wrapper (§7): first run records request-hash→response transcripts; replay runs never touch the network. Same seed + same transcripts ⇒ identical `metrics.json`. Live first-run responses are the explicit, documented non-deterministic exception. | spec01 §3 D40. |
| D41 | *(v2)* **No time-decay.** None of the three benchmarks has query timestamps ⇒ demand half-life decay is off (uniform recency weights); stated as a threat in §9. | spec01 §3 D41. |
...
- [x] Write `pyproject.toml`: project metadata (D1), core deps (Global Constraints), extras `notebooks/optimization/bm25/st`, dev group (`pytest`, `pytest-cov`, `ruff`, `mypy`, `scipy-stubs`, `types-pyyaml`), `[project.scripts] ragsynth = "ragsynth.cli:app"`, uv_build backend, ruff config (line 100, `select=["ALL"]`, google docstrings, documented ignores: `COM812`,`ISC001` (formatter), `D203`,`D213` (convention pair), `ANN401`, `PLR0913`, `TC001-003` pragmatic; per-file-ignores `tests/**`: `S101`,`PLR2004`,`D`,`ANN`,`SLF001`; `cli.py`: `T201` not needed—no prints, rich console), mypy strict + pydantic plugin + `ignore_missing_imports` overrides (`sklearn.*`, `joblib.*`, `dspy.*`, `textgrad.*`, `bm25s.*`, `sentence_transformers.*`), pytest config (`testpaths=["tests"]`).
- [x] `src/ragsynth/__init__.py` with `__version__ = "0.1.0"` + module docstring; empty `py.typed`.
- [x] `uv sync` → lockfile; `uv run python -c "import ragsynth"` works.
- [x] Run all four §15.3 commands (pytest passes with no tests via `--cov-fail-under=0`? NO — keep 70: add one smoke test `tests/test_version.py::test_version`); `ragsynth run` will fail until T3.8 — acceptable gap, note in commit; the §15.3-at-every-commit rule binds fully once the CLI exists.
- [x] Commit `chore: bootstrap uv project, toolchain, strict lint/type config`.
...
- [x] Implement; run; commit `feat(io): EmbeddingStore, ArtifactStore with sha256 manifest, optional-dep helper`.
...
- [x] Failing tests: frozen-ness (assignment raises), `Stratum.key` ordering (`{"b":"2","a":"1"}` ⇒ `"a=1|b=2"`), `Chunk.create` id determinism + 16-hex shape, `AnnotationRecord` JSON round-trip via pydantic, `Turn` role literal validation.
- [x] Implement all 11 modules + `domain/__init__.py` re-exports; commit `feat(domain): frozen pydantic v2 domain model (conversation- and visual-ready fields)`.
...
- [x] Failing tests: register/get/duplicate-key error; unknown key error message contains `known` + every registered key; `Resources.rng("a")` deterministic across instances and ≠ `rng("b")`; `with_overrides` swaps one field, keeps rest; `PipelineState` field defaults independent between instances.
- [x] Implement; note `Resources` uses `TYPE_CHECKING` imports + string annotations to avoid cycles (protocols imported from adapters bases; sampling imported lazily for types only).
- [x] Commit `feat(pipeline): registries, Resources, PipelineState, PipelineStep ABC`.
...
- [x] Failing tests: mock determinism (two instances, same inputs ⇒ same outputs); `MockEmbedder` rows unit-norm; retriever returns exact top-k (hand fixture: 4 vectors, known cosines, k=2); Protocol conformance via `isinstance` checks with `runtime_checkable` OFF — assert structurally by passing mocks where Protocol is annotated (mypy covers it) + a `tests/pipeline/test_contract.py` parametrized adapter-satisfies-mock check (§15.2).
- [x] Commit `feat(adapters): protocols, deterministic mocks, dense in-memory retriever`.
...
- [x] Tests FIRST (each check: pass fixture + reject fixture + reason string):
...
- [x] Implement + `gate/checks/__init__.py` imports all; commit `feat(gate): five verification checks (dedup, zero-context, answerability, round-trip, uniqueness)`.
...
- [x] Tests FIRST: `ragsynth validate --config configs/v1_toy.yaml` exit 0 + summary table; broken type ⇒ exit 1, message lists known keys; `ragsynth run --config <tiny toy config>` (conftest-written, n small) produces `metrics.json`/`report.md`/`figures/`/`artifacts/` + records jsonl; `ragsynth report --config <same>` re-renders report.md from stored metrics.json (delete report.md first, rerun, identical bytes).
- [x] Write both real configs (v1_toy: §13 shape + D11 adapter types, `reuse_pipeline_for: a1`, validator `arm_params` carrying D12 emission overrides; v1_local_corpus: jsonl dataset, hashed embedder dim 256, mock chat/judge, arms [a0,a1,oracle] — a2 included too: hashed space is a valid planning space); rich console output, logging config (INFO default, `--verbose`).
- [x] Commit `feat(cli): run/validate/report commands + v1 configs`.
...
- [x] Write the qualitative assertions (CI world: d=32, n_prod=1500, n_per_arm=200, n_boot=200, fixed seed): `wc2st(a2) < wc2st(a1) − 0.10` and `wc2st(a1) > 0.80`; `mmd(a2) < mmd(a1) < mmd(a0)`; `kl(a0) > kl(a1) ≥ kl(a2)`; `ess(a1) ≥ ess(oracle) − 0.15` and `ess(a1) > ess(a0)`; `tau(a2) > tau(a1)`, `tau(oracle) ≥ tau(a2) − 0.05`, gates_passed(a0) is False; controls: `p_drop < 0.05` for all arms, `p_noise < 0.05` for a2 & oracle, `p_noise > 0.05` for a1 (the "A1 misses the noise control" claim).
- [x] Iterate emission/judge/κ params (documented knobs: per-arm style/noise D12, `kappa_chunk=150`, `kappa_query=400`, zoo σ set, noise-control σ=0.5, τ_ans) until stable at the fixed seed **and** at seeds {0,1,2} (run thrice locally; assert only seed 0 in CI). Record the tuning trace in `experiments/v1/report.md` findings.
- [x] Commit `test(e2e): toy-world 4-arm qualitative table locked`.
...
- [x] Commit `docs: README quickstart + ARCHITECTURE with extension recipe`.

### Phase 5 — Final verification & adversarial review
- [x] Full §15.3 sweep from clean checkout (`git stash -u` sanity): all four commands.
...
- [x] Final commit `chore: v1 acceptance` + summary against §15 checklist.
