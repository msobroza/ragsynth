...
| D37 | *(v2)* **Cluster-count ladder.** Default `C=8`; descend over `{8,6,4,2}` while (a) every cluster has ≥ 30 anchor queries and (b) `n_seeds · q_c ≥ 30` per cluster (`n_seeds=240`, λ=0.7) — evaluated a priori, deterministically; chosen `C` recorded in `metrics.json` and the report header; a runtime stratum still below 30 falls back to the v1 SPEC §8 wC2ST floor rule (flagged, skipped). | spec01 §3 D37. |
